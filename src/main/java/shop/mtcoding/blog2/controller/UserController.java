...
public class UserController {
    ...
    public @ResponseBody ResponseDto<?> check(String username){     	
        CheckValid.check(username, "username을 입력해주세요");
        CheckValid.check(username, "동일한 username이 존재");
        return service.중복체크(username);
    }    

    @PostMapping("/join")
    public String userJoin(@Validated UserJoinDto userDto){
        CheckValid.check(userDto.getUsername(), "아이디를 입력하세요");
        CheckValid.check(userDto.getPassword(), "패스워드를 입력하세요");
        CheckValid.check(userDto.getEmail(), "이메일을 입력하세요");
    ...
    public @ResponseBody String update(UserUpdateReqDto updateReqDto){
        CheckValid.checkApi(principal(), "로그인이 필요합니다.", HttpStatus.UNAUTHORIZED);
    ...
    public ResponseEntity<?> profileUpdate(MultipartFile profile) throws Exception{ 
        CheckValid.checkApi(principal(), "로그인이 필요한 페이지 입니다.", HttpStatus.UNAUTHORIZED);
        CheckValid.checkApi(profile, "사진이 전송 되지 않았습니다.");
